...
// The signature is kept compatible with jasmine.fireMouseEvent so that clickMouse
// could be used as a drop-in replacement; we're ignoring the `type` argument though.
// mouseClick is just a helpful alias since I always forget which way it is named. ;)
jasmine.clickMouse = jasmine.mouseClick = function(target, type, x, y, button) {
    var el = Ext.get(target);
    
    x = x != null ? x : el.getLocalX();
    y = y != null ? y : el.getLocalY();
    
    button = button != null ? button : 0;
    
    if (Ext.supports.PointerEvents) {
        jasmine.firePointerEvent(el.dom, 'pointerdown', 1, x, y, button);
        jasmine.firePointerEvent(el.dom, 'pointerup',   1, x, y, button);
    }
    else if (Ext.supports.MSPointerEvents) {
        jasmine.firePointerEvent(el.dom, 'MSPointerDown', 1, x, y, button);
        jasmine.firePointerEvent(el.dom, 'MSPointerUp',   1, x, y, button);
    }
    else {
        jasmine.fireMouseEvent(el.dom, 'mousedown', x, y, button);
        jasmine.fireMouseEvent(el.dom, 'mouseup',   x, y, button);
    }
    
    jasmine.fireMouseEvent(el.dom, 'click', x, y, button);
}
