describe("Ext.event.publisher.Dom", function(){
    var DomPublisher = Ext.event.publisher.Dom,
        publisher, dispatcher;

    xdescribe("members", function(){
        beforeEach(function() {
...
//            });
    ...
    describe("delegated event order", function() {
        itFiresMouseEvents("should fire delegated events in DOM order", function() {
            function fire(el) {
                order = [];
                var target = el.down('.foo');
            ...
            Ext.destroy(child1_1, child1, child2, root);
        });
    });
});
