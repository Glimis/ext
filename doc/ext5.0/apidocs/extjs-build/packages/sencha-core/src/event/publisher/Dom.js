...
Ext.define('Ext.event.publisher.Dom', {
    ...
    subscribe: function(target, eventName, options, observable) {
        var me = this,
            ...
            directSubscribers, dom, type, value, selectorRoot;
        ...
        if (idOrClassSelectorMatch) {
            ...
            if (target === this.SELECTOR_ALL) {
                ...
                // We need to store the root for the selector so that we can control
                // the order in which the delegates fire. See doPublish.
                selectorRoot = target.match(me.idRe);
                selectorSubscribers.push({
                    root: selectorRoot[1],
                    selector: target
                });
            }
    ...
    unsubscribe: function(target, eventName, all, options, observable) {
        var me = this,
            ...
            selectorSubscribers, directSubscribers, type, value, dom, id, selectorItem;
        ...
        if (idOrClassSelectorMatch) {
            ...
            if (target === me.SELECTOR_ALL) {
                ...
                selectorItem = selectorSubscribers[target];
                delete selectorSubscribers[target];
                Ext.Array.remove(selectorSubscribers, selectorItem);
            }
        }
    ...
    doPublish: function(subscribers, eventName, targets, event, capture) {
        var me = this,
            ...
            args = [event],
            hasDispatched = false,
            classNameSplitRegex = me.classNameSplitRegex,
            start = 0,
            end = targets.length,
            increment = 1,
            ...
            if (hasIdSubscribers) {
                ...
                if (id) {
                    if (idSubscribers.hasOwnProperty(id)) {
                        hasDispatched = true;
                        me.dispatch('#' + id, eventName, args, capture);
                    }
                }
            }
                    ...
                    for (j = 0,subLn = classNames.length; j < subLn; j++) {
                        ...
                        if (!isClassNameHandled[className]) {
                            ...
                            if (classNameSubscribers.hasOwnProperty(className)) {
                                hasDispatched = true;
                                me.dispatch('.' + className, eventName, args);
                            }
                        }
                    }
                }
        ...
        if (hasAllSubscribers && !hasDispatched) {
            ...
            hasDispatched = true;
            me.dispatch(me.SELECTOR_ALL, eventName, args);
            if (event.isStopped) {
                return hasDispatched;
            }
        }
        ...
        if (hasSelectorSubscribers) {
            for (j = 0,subLn = targets.length; j < subLn; j++) {
                ...
                // We need to sort the delegates so that the order in which they are
                // compared reflects the DOM order. Consider:
                // <div id="root">
                //     <div id="child1">
                //         <div class="foo">
                //     </div>
                ...
                // </div>
                // 
                // If we do not sort the selectors, they will fire in the order in
                // which they are bound, so the root could fire before the children. As such,
                // sort them so that dom nodes in the same subtree have lower nodes first.
                selectorSubscribers = Ext.Array.clone(selectorSubscribers);
                Ext.Array.sort(selectorSubscribers, me.sortSelectorSubscribers);
                for (i = 0,ln = selectorSubscribers.length; i < ln; i++) {
                    selector = selectorSubscribers[i].selector;

                        if (Ext.fly(target).is(selector)) {
                            event.setCurrentTarget(target);
                            hasDispatched = true;
                            me.dispatch(selector, eventName, args);
                        }
    ...
    sortSelectorSubscribers: function(a, b) {
        var out = 0,
            aRoot = a.root,
            bRoot = b.root,
            position;
