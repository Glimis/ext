...
Ext.define('Ext.grid.plugin.BufferedRenderer', {
    ...
    onViewRefresh: function(view, records) {
        ...
        targetEl = view.getTargetEl();

        // Naturally tabbable elements rendered into the grid must be made *programatically* focusable.
        // Allow current operation to finished quickly by deferring that.
            ...
            if (elCount) {
                ...
                if (!(start === rows.startIndex && end === rows.endIndex)) {
                    ...
                    // View is shrinking: remove rows from end
                        ...
                        // Inform listeners about item removal
    ...
    onReplace: function(store, startIndex, oldRecords, newRecords) {
        ...
        if (!renderedSize || startIndex <= me.getLastVisibleRowIndex() + me.leadingBufferZone || (renderedSize < me.viewSize && newRecords.length > oldRecords.length) || storeSize < renderedSize) {
            me.refreshView();
            renderedSize = rows.getCount();
    ...
    scrollTo: function(recordIdx, options) {
        ...
        if (record && (targetRow = view.getNode(record))) {
            //\\ TODO: Use the view component's Scroller API when it exists (to support touch scrolling platforms)
            Ext.fly(targetRow).scrollIntoView(view.getTargetEl(), null, animate, highlight);
        ...
        store.getRange(startIdx, endIdx, {
            callback: function(range, start, end) {
                ...
                if (direction === 1) {
                    //\\ TODO: use view component's Scroller API to get the clientHeight information
                    scrollDecrement = viewDom.clientHeight - targetRow.offsetHeight;
                }
                //\\ TODO: use view component's Scroller API to get the scroll information used in this calculation
                me.position = me.scrollTop = Math.min(Math.max(0, tableTop - view.body.getOffsetsTo(targetRow)[1]) - scrollDecrement, viewDom.scrollHeight - viewDom.clientHeight);
                //\\ TODO: Use the view component's Scroller API when it exists (to support touch scrolling platforms)
                //\\ Ensure that tha API allows animation and callbacks.
    ...
    refreshView: function(startIndex) {
        ...
        endIndex = Math.min(Math.max(rows.startIndex + viewSize - 1, me.getLastVisibleRowIndex() + me.leadingBufferZone), maxIndex);
    ...
    doRefreshView: function(range, startIndex, endIndex, options) {
        ...
        if (view.refreshCounter) {
            ...
            if (range.length) {
                ...
                if (viewMoved) {
                    if (startIndex > rows.startIndex) {
                        calculatedTop += rows.item(startIndex, true).offsetTop;
                    } else {
                        ...
                        if (me.variableRowHeight) {
                            ...
                            calculatedTop = startIndex * me.rowHeight - me.tableTopBorderWidth;
                        }
                    }
            ...
            if (options.focusPosition) {
                navModel.setPosition(options.focusPosition, null, null, null, true);
            }
